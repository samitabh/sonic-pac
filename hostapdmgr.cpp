...
{/*CG_PAC*/
    ...
    if (stat(HOSTAPDMGR_HOSTAPD_USER_CONFIG_FILE_PATH, &buffer) == 0) {
        // The file exists, so we can attempt to delete it
        if (remove(HOSTAPDMGR_HOSTAPD_USER_CONFIG_FILE_PATH.c_str()) == 0) {
            SWSS_LOG_INFO("Deleted Hostapd User Config file: %s", HOSTAPDMGR_HOSTAPD_USER_CONFIG_FILE_PATH.c_str());
        } else {
            SWSS_LOG_ERROR("Failed to delete Hostapd User Config file: %s", HOSTAPDMGR_HOSTAPD_USER_CONFIG_FILE_PATH.c_str());
        }
    } else {
        SWSS_LOG_INFO("Hostapd User Config file does not exist at: %s", HOSTAPDMGR_HOSTAPD_USER_CONFIG_FILE_PATH.c_str());
    }
}
